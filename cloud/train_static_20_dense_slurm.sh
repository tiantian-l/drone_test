#!/bin/bash
# Submit from the repository root: sbatch cloud/train_static_20_dense_slurm.sh
#SBATCH --job-name=drone_20_dense_s0
#SBATCH --nodes=1
#SBATCH --gpus=1
#SBATCH --ntasks=1
...
if [ ! -f cloud/run_static_factorial.sh ]; then
  echo "Submit from the repository root: sbatch cloud/train_static_20_dense_slurm.sh" >&2
  exit 1
fi
